...
def _load_log_level_from_config() -> int:
    """
    Load log_level from config_grounding.json and convert to MCPNEXT_DEBUG value.
    ...
    try:
        ...
        if config_path.exists():
            with open(config_path, 'r', encoding='utf-8') as f:
                ...
                # Convert log level string to MCPNEXT_DEBUG value
                level_map = {
                    "DEBUG": 2,
                    "INFO": 1,
                    "WARNING": 0,
...
MCPNEXT_DEBUG = _load_log_level_from_config()

# Default log directory and file pattern
# Use absolute path to mcpnext/logs directory
DEFAULT_LOG_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "logs")
DEFAULT_LOG_FILE_PATTERN = "mcpnext_{timestamp}.log"
...
class Logger:
    ...
    3. Dynamically adapts log levels according to ``MCPNEXT_DEBUG``.
    """

    _ROOT_NAME = "mcpnext"        # Package root name
    ...
    def _get_default_log_file() -> str:
        ...
        - logs/<script_name>/mcpnext_2025-10-24_15-30-00.log
        """
        # Get the name of the main script
        script_name = "mcpnext"  # Default name
        try:
            import __main__
    ...
    def get_logger(cls, name: Optional[str] = None) -> logging.Logger:
        """Return a logger with *name* (defaults to ``mcpnext``).
    ...
    def configure(
        ...
        *,
        ...
        with cls._lock:
            ...
            # Select the logger to attach handlers to (root logger or mcpnext).
            target_logger = (
    ...
    def set_debug(cls, debug_level: int = 2) -> None:
        ...
        global MCPNEXT_DEBUG
        MCPNEXT_DEBUG = max(0, min(debug_level, 2))
    ...
    def add_file_handler(
        ...
        """
        Append a file handler to the given (default ``mcpnext``) logger.
    ...
    def _resolve_level(cls, level: Optional[int]) -> int:
        ...
        return {2: logging.DEBUG, 1: logging.INFO}.get(MCPNEXT_DEBUG, logging.WARNING)
...
# ``MCPNEXT_DEBUG`` (preferred) or legacy ``DEBUG`` environment variable.
_env_debug = os.getenv("MCPNEXT_DEBUG") or os.getenv("DEBUG")
...
# Get mcpnext logger for internal logging
logger = Logger.get_logger()
logger.debug("MCPNext logging initialized")