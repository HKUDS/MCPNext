...
from mcpnext.local_server.utils import AccessibilityHelper, ScreenshotHelper
from mcpnext.local_server.platform_adapters import get_platform_adapter
from mcpnext.local_server.health_checker import HealthChecker
from mcpnext.local_server.feature_checker import FeatureChecker
...
def health_check():
    ...
    return jsonify({
        'status': 'ok',
        'service': 'MCPNext Desktop Server',
        'version': '1.0.0',
...
def print_banner(host: str = "127.0.0.1", port: int = 5000, debug: bool = False):
    ...
    from mcpnext.utils.display import print_banner as display_banner, print_section, print_separator, colorize
    
    # STARTUP INFORMATION
    display_banner("MCPNext · Local Server")
...
def main():
    import argparse
    from mcpnext.config.utils import get_config_value
    
    parser = argparse.ArgumentParser(
        description='MCPNext Local Server - Desktop Control Server'
    )
